...
type TipoEvento = 'cultural' | 'lanzamiento' | 'reunion' | 'otro';
// Mapea los tipos de tu API a los tipos visuales
const tiposEvento: Record<TipoEvento, {
  color: string;
...
const CalendarApp = ({ eventos }: { eventos: any[] }) => {
  ...
  const [eventoSeleccionado, setEventoSeleccionado] = useState<any | TipoEvento | null>(null)
  ...
  const eventosNormalizados = eventos.map(e => ({
    ...
    title: e.Titulo,
    start: toScheduleXFormat(e.Start || e.start || e.publishedAt),
    end: toScheduleXFormat(e.End_time || e.end || e.Start || e.publishedAt),
  ...
  const tiposUnicos = Array.from(new Set(eventosNormalizados.map(e => e.type))).filter(Boolean);
  ...
  const calendar = useCalendarApp({
      ...
      tiposUnicos.map((tipo: TipoEvento) => [
        tipo,
        {
          ...
          lightColors: {
            ...
            container: tiposEvento[tipo]?.backgroundColor || tiposEvento.otro.backgroundColor,
            onContainer: tiposEvento[tipo]?.borderColor || tiposEvento.otro.borderColor,
          },
          darkColors: {
            main: tiposEvento[tipo]?.color || tiposEvento.otro.color,
            container: tiposEvento[tipo]?.backgroundColor || tiposEvento.otro.backgroundColor,
    ...
    callbacks: {
      onEventClick(calendarEvent) {
        ...
        const eventoCompleto = eventosNormalizados.find(e => e.id === eventoId)
  ...
  const eventosFiltrados = filtrosActivos.length > 0
    ? eventosNormalizados.filter(evento => filtrosActivos.includes(evento.type))
    : eventosNormalizados
  ...
  return (
    <div className="w-full space-y-6">
            ...
            {tiposUnicos.map((tipo: TipoEvento) => {
              ...
              return (
                <div
                  ...
                  className={`flex items-center gap-3 p-3 rounded-lg border cursor-pointer transition-all duration-200 ${isActive
                    ? 'shadow-lg scale-105'
                    : 'opacity-80 hover:opacity-100 hover:shadow-md'
                    }`}
                  style={{
                    backgroundColor: isActive ? tipoData.backgroundColor : `${tipoData.color}10`,
                ...
                >
                  ...
                  ></div>
                  <span className="text-sm font-medium" style={{ color: tipoData.color }}>
                    {tipoData.label}
                  </span>
                </div>
              )
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                ...
                <div
                  ...
                  style={{
                    backgroundColor: `${tiposEvento[eventoSeleccionado.type as TipoEvento]?.backgroundColor || tiposEvento.otro.backgroundColor}`,
                    color: tiposEvento[eventoSeleccionado.type as TipoEvento]?.color || tiposEvento.otro.color
                  }}
                >
                  {tiposEvento[eventoSeleccionado.type as TipoEvento]?.label || tiposEvento.otro.label}
                </div>
              </div>
            </div>
      ...
      <div className="text-sm text-muted-foreground space-y-1">
        ...
        <p>🗓️ El calendario muestra {eventosFiltrados.length} de {eventosNormalizados.length} eventos programados</p>
        <div className="mt-2 text-xs">
          <strong>Distribución:</strong>
          {tiposUnicos.map((tipo: TipoEvento) => {
            const count = eventosNormalizados.filter(e => e.type === tipo).length
            const tipoData = tiposEvento[tipo] || tiposEvento.otro;
            return (
